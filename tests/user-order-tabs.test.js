...
test('exposes six user order tabs in requested order', () => {
...
test('maps user order statuses into the expected tab groups', () => {
  ...
  assert.equal(orderTabs.tabOfUserStatus(3), 'paid');
  assert.equal(orderTabs.tabOfUserStatus('WAITING_SERVICE'), 'paid');
  assert.equal(orderTabs.tabOfUserStatus(4), 'serving');
  ...
  assert.equal(orderTabs.tabOfUserStatus('REJECTED_CLOSED'), 'closed');
  assert.equal(orderTabs.tabOfUserStatus('REFUNDING'), 'closed');
...
test('returns numeric query status groups for each tab', () => {
  ...
  assert.deepEqual(orderTabs.statusesOfUserTab('paid'), [3]);
...
test('keeps legacy tab helper aliases on the same numeric behavior', () => {
  ...
  assert.equal(orderTabs.tabOfStatus('CANCELED'), 'closed');
});
